...
class TransformerNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.d_embedding = nn.Embedding(
            int(np.log2(diffusion_steps)) + 1, self.hidden_size
        )

        if self.input_dims != config.hidden_size:
            self.input_up_proj = nn.Sequential(
                nn.Linear(self.input_dims, config.hidden_size),
                nn.Tanh(),
    ...
    def get_logits(self, hidden_repr):
        if self.logits_mode == 1:
            ...
            text_emb_t = th.transpose(text_emb.view(-1, text_emb.size(-1)), 0, 1)  # shoruct_size, bsz*seqlen
            arr_norm = (text_emb ** 2).sum(-1).view(-1, 1)  # bsz*seqlen, 1
            dist = emb_norm + arr_norm.transpose(0, 1) - 2.0 * th.mm(self.lm_head.weight,
                                                                     text_emb_t)  # (vocab, shoruct_size) x (shoruct_size, bsz*seqlen)
    ...
    def forward(self, x, time_steps, D=None):
        """
        ...
        """
        emb_t = self.time_embed(timestep_embedding(time_steps, self.hidden_t_dim))
        D_shifted = torch.where(D == 0, torch.tensor(1, device=D.device), D.to(torch.int) << 1)
        emb_d = self.d_embedding(torch.log2(D_shifted).to(torch.int))
        ...
        emb_inputs = (
                ...
                emb_d.unsqueeze(1).expand(-1, seq_length, -1)
        )
