...
def create_criterion(model, training_cfg: TrainingConfig, tokenizer) -> CompositeCriterion | FlowNllCriterion:
    """Create criterion based on training config."""
...
if __name__ == '__main__':
    ...
    # Initialize model
    model = TransformerNetModelFactory(gen_cfg.training_config.model).build()

    # Define Criterions
    tokenizer = AutoTokenizer.from_pretrained(gen_cfg.training_config.model.config_name)
    criterion = create_criterion(model, gen_cfg.training_config, tokenizer)
    ...
    # Add EMA callback if configured and requested
    if gen_cfg.training_config.ema is not None and gen_cfg.use_ema_weights:
