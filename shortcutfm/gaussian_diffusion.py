...
from .utils.nn import mean_flat, mean_with_mask
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None, sc_rate=0):
        ...
        if sc_rate > 0: # use self conditioning
            ...
            if th.rand(1)[0] < sc_rate:
                t_next = t + 1
                x_t_next = x_start + (noise-x_start)*th.broadcast_to(t_next.float() * (1.0 / self.num_timesteps), \
                                    (x_start.size(1), x_start.size(2), x_start.size(0))).permute(2,0,1)
                x_0_hat = model(th.cat((x_t_next, x_0_hat), dim=-1), self._scale_timesteps(t_next), **model_kwargs).detach() # rescale is implemented by wrapped model
                x_0_hat = th.where(input_ids_mask_reshape==0, x_start, x_0_hat)
            x_t = th.cat((x_t, x_0_hat), dim=-1)

        get_logits = model.model.module.get_logits

        # set D_mask to 0 in case "D" not in model_kwargs
        D_mask = th.zeros_like(input_ids_mask)
        # Self-consistency loss for D > 0
        if "D" in model_kwargs:
            D_values = model_kwargs["D"]  # Extract batch-wise D values
            D_mask = (D_values > 0).float() # Mask for examples where D > 0

            d = D_values
            t_d = t + d
            model_kwargs["D"] = d  # Ensure D is passed to model

            # Compute s_t = s_theta(x_t, t, shoruct_size)
            s_t = model(x_t, self._scale_timesteps(t), **model_kwargs)

            # Step forward: x_t+shoruct_size = x_t + s_t * shoruct_size
            x_t_d = x_t + s_t * d.unsqueeze(-1)

            # Compute s_t+shoruct_size = s_theta(x_t+shoruct_size, t+shoruct_size, shoruct_size)
            s_t_d = model(x_t_d, self._scale_timesteps(t_d), **model_kwargs)

            # Compute self-consistency target: stopgrad((s_t + s_t+shoruct_size) / 2)
            s_target = ((s_t + s_t_d) / 2).detach()

            # Compute MSE loss for D > 0: ||s_theta(x_t, t, 2d) - s_target||^2
            s_2d = model(x_t, self._scale_timesteps(t), **model_kwargs)
            self_consistency_loss = mean_with_mask((s_2d - s_target) ** 2, D_mask)
        else:
            self_consistency_loss = 0

        # Standard loss (for D = 0)
        if self.predict_xstart:
        ...
        standard_mse = mean_with_mask((target - model_output) ** 2, mse_mask)

        terms = {"mse": standard_mse * (1 - D_mask) + self_consistency_loss * D_mask,
                 "nll": self._token_discrete_loss(model_output, get_logits, input_ids_x, mask=input_ids_mask,
                                                  truncate=True, t=t)}
        terms["loss"] = terms["mse"] + terms["nll"]
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, **kwargs):
        ...
        # # print(new_ts)
        # if self.rescale_timesteps:
        #     new_ts = new_ts.float() * (1.0 / self.original_num_steps)
